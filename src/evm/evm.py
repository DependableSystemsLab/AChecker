...
def run_symbolic(program, path, code=None, state=None, ctx=None, inclusive=False, mode=None, code_path=None, ac_jumpi=None):
    ...
    max_timestamp = (datetime.datetime(2025, 1, 1) - datetime.datetime(1970, 1, 1)).total_seconds()    
    ...
    #traget_fun = code_info(program,path)
    ...
    while state.pc in program:
        ...
        if ((inclusive and len(path) == 0)
            ...
            return SymbolicResult(xid, state, constraints, sha_constraints, target_op, pib)
        ...
        # VM state manipulations
            ...
            if op == 'POP':
                ...
                s0 = stk.pop()                
                if mode=='FIntendedB':                    
                    stk.append(ctx_or_symbolic('STORAGE', ctx, xid))
                else:
                ...
                if concrete(s1):
                    ...
                    if state.pc + 1 == next_target:
                        if not (concrete(s0) and s0 == next_target): #Fall through target                                                                                                                                                                     
                            #if mode=='FIntendedB':
                            #    print(ins)
                            if mode=='FIntendedB' and ins.bb.start in code_path and ins.addr not in ac_jumpi and not set(['CALLDATALOAD','CALLDATACOPY','CALLDATASIZE'])&set([s.name for s in ins.bb.ins]):                                                                                                                    
                                constraints.append(s1 != 0) #original s1==0                                                                                                              
                                pib=True          
                                #print('ok')           
                            else:                                
                                constraints.append(s1 == 0) #original s1==0                                              
                    elif concrete(s0) and s0 == next_target:
                        if state.pc + 1 != next_target: #Jump target                                                                                                                                                                                        
                            #if mode=='FIntendedB':
                            #    print(ins)
                            if mode=='FIntendedB' and ins.bb.start in code_path and ins.addr not in ac_jumpi and not set(['CALLDATALOAD','CALLDATACOPY','CALLDATASIZE'])&set([s.name for s in ins.bb.ins]):                                                                                            
                                constraints.append(s1 == 0) #original s1 !=0                            
        ...
        # Calls
            ...
            if concrete(s1) and s1 <= 8:
                if s1 == 4:
                    logging.debug("Calling precompiled identity contract")
                ...
                logging.debug("Calling contract %s (%d_%d)" % (s1, instruction_count, xid))
